...
import { StaggerItem } from "@/components/common/MotionWrapper";
      ...
      <div className="w-full h-full px-5">
        ...
        <div className="mt-20 grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-8">
          {projects.map((project, index) => {
            ...
            const columns = useGridColumns({ base: 1, lg: 2, xl: 3 });
            return (
              <StaggerItem key={id} animateIndex={index % columns}>
                <ThreeDCard
                  outterClassName="h-full w-full"
                  className="h-full flex flex-col bg-tertiary p-5 rounded-2xl overflow-hidden"
                >
                  {/* top */}
